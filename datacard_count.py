...
def datacard(cat, sign):
    ...
    if useFilter:
        ...
        fnoweight = TFile("rootfiles_"+options.name+"_noweight/"+cat+".root", "READ")
        if 'AH' in cat:
            ...
            bkgFilterList = []
        for bkg in back:
            N = fnoweight.Get(bkg).GetBinContent(1)
            if N < 5:
                continue
   ...
   #  exit()
    ...
    card += "observation" + space + ("%-25.6f" % getNumber(cat, 'data_obs')) + "\n"
    card += hline
...
#                    if i>0: card += ("%-20.1f" % 1)
#                    else: card += "%-25s" % "-"
                if not checkShape(cat, s, h): 
                    down, up = getShapeContent(cat, s, h)
                    card += ("%.3f/%-19.3f" % (down, up))
                else: card += ("%-25.0f" % 1)
            card += "\n"
    ...
    for process in [sign] + backTemp:
        nEntries, alpha = getGammaEntries(cat, process)
        if nEntries < 5:
            continue
        card += "%-55s gmN  %-7.0f " % (process+"_"+cat+"_"+year, nEntries)
        for s in [sign] + backTemp:
            if s == process:
                card += "%-25.20f" % alpha
            else:
                card += "%-25s" % "-"
...
def getShapeContent(cat, s, syst=''):
    f = TFile("rootfiles_"+options.name+"/"+cat+".root", "READ")
    h = f.Get(s).GetBinContent(1)
    hUp = f.Get(syst+'Up'+'/'+s).GetBinContent(1)
    hDown = f.Get(syst+'Down'+'/'+s).GetBinContent(1)

    #Symmetrize up/down variations
    unc = ((hUp/h) - (hDown/h))/2.0
    down = max(1.0 - unc, 0.01)
    up = max(1.0 + unc, 0.01)
    # down = hDown/h
    # up = hUp/h
...
def getGammaEntries(cat, s):
    f = TFile("rootfiles_"+options.name+"/"+cat+".root", "READ")
    fnoWeight = TFile("rootfiles_"+options.name+"_noweight/"+cat+".root", "READ")
    integral = f.Get(s).GetBinContent(1)
    nEntries = fnoWeight.Get(s).GetBinContent(1)
    if nEntries == 0:
        alpha = 1
        print "Signal process %s in file %s has 0 nEntries, setting nEntries = 0 and alpha = 1" % (s,"rootfiles_"+options.name+"/"+cat+".root")
    else:
        alpha = integral/nEntries

    return nEntries, alpha
