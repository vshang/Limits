...
from ROOT import TLegend, TLatex, TText, TLine, TBox, TMath, Double
...
def fixDataRatio(hist, useGarwood=True, cutGrass=False, maxPoisson=False):
    if hist==None: return
    varBins = False
    data = TGraphAsymmErrors()
    alpha = 1 - 0.6827
...
def printTable(hist, sign=[]):
    ...
    for i, s in enumerate(['data_obs']+samples+['BkgSum']):
        ...
        err = Double(0.)
        nBin = hist[s].GetNbinsX()
        integral = hist[s].IntegralAndError(1,nBin,err)
        print "%-20s" % s, "\t%-10.2f" % integral,"$\pm$ \t%-10.2f" %err
    print "-"*60
    for i, s in enumerate(sign):
        #if not sample[s]['plot']: continue
        err = Double(0.)
        nBin = hist[s].GetNbinsX()
        integral = hist[s].IntegralAndError(0,nBin,err)
        print "%-20s" % s, "\t%-10.2f" % integral, "$\pm$ \t%-10.2f" %err
    print "-"*60
...
def getChannel(channel):
    ...
    elif 'bb' in channel or '2b' in channel: text += ", 2 b tag" if isAH else ", 2 b tag"
    elif 'b' in channel or '1b' in channel: text += ", 1 b tag" if isAH else ", 1 b tag"
    # region
    if 'SR' in channel: text += ", SR"
    elif 'SB' in channel: text += ", sidebands region"
    elif 'TR' in channel: text += ", top CR"
    elif 'WR' in channel: text += ", W CR"
    elif 'ZR' in channel: text += ", Z CR"
    elif 'MC' in channel: text += ", simulation"
    elif 'Inc' in channel: text += ", inclusive region"

    if '0f' in channel: text += ", 0 FJ"
    elif '1f' in channel: text += ", 1 FJ"
...
def drawAnalysis(s, center=False):
    ...
    if s.startswith('X'):
        ...
        # if 'AH' in s: text += "(All-hadronic channel)"
        # if 'SL' in s: text += "(Single-lepton channel)"
    else: return True
    latex = TLatex()
    latex.SetNDC()
...
def drawCMS(lumi, text, onTop=False):
    ...
    if (type(lumi) is float or type(lumi) is int):
        if float(lumi) > 0: latex.DrawLatex(0.95, 1., "%.1f fb^{-1}  (13 TeV)" % (float(lumi)/1000.))
    ...
    elif type(lumi) is str: latex.DrawLatex(0.95, 0.998, "%s  (13 TeV)" % lumi)
    ...
    if not onTop: latex.DrawLatex(0.14, 0.9 if len(text)>0 else 0.85, "CMS")
...
def setBotStyle(h, r=4, fixRange=True):
    ...
    if fixRange:
        ...
        for i in range(1, h.GetNbinsX()+1):
            if abs(h.GetBinContent(i))<1.e-10:
                h.SetBinContent(i, -1.e-10)
...
def setFitBotStyle(h, r=5.5, fixRange=True):
    ...
    h.GetYaxis().SetTitleSize(h.GetYaxis().GetTitleSize()*(r-1.6));
    #h.GetYaxis().SetTitleOffset(h.GetYaxis().GetTitleOffset()/(r-1.8));
    #h.GetYaxis().SetTitleOffset(h.GetYaxis().GetTitleOffset()/(r-1.7));
    h.GetYaxis().SetTitleOffset(h.GetYaxis().GetTitleOffset()/(r-1.8));
    if fixRange:
        #h.GetYaxis().SetRangeUser(-0.3, 2.3)
        h.GetYaxis().SetRangeUser(-0.5, 3.5)
        for i in range(1, h.GetNbinsX()+1):
            if abs(h.GetBinContent(i))<1.e-6:
                h.SetBinContent(i, -1.e-6)
            # if h.GetBinContent(i)<1.e-6:
            #     h.SetBinContent(i, -1.e-6)
...
def setFitResStyle(h, r=5., fixRange=True):
    ...
    h.GetXaxis().SetLabelOffset(h.GetXaxis().GetLabelOffset()*(r-1));
    #h.GetXaxis().SetTitleSize(h.GetXaxis().GetTitleSize()*(r-1));
    h.GetXaxis().SetTitleSize(h.GetXaxis().GetTitleSize()*(r-0.5));
    h.GetXaxis().SetTitleOffset(h.GetXaxis().GetTitleOffset()*1.1);
    h.GetYaxis().SetLabelSize(h.GetYaxis().GetLabelSize()*(r-1));
    h.GetYaxis().SetNdivisions(205);
    h.GetYaxis().SetTitleSize(h.GetYaxis().GetTitleSize()*(r-1.06));
