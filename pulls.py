...
def pullsVertical(fileName):
    ...
    #content_nostat = [x for x in content if not 'CMS_stat' in x]
    content_nostat = [x for x in content if not 'bin' in x]
    ...
    h_pulls = TH2F("pulls", "", 10, -5., 5., nbins, 0, nbins)
    ...
    c1 = TCanvas("c1", "Pulls", 600, 600+nbins*10)#nbins*20)
    c1.cd()
    c1.SetGrid(0, 1)
...
#    drawRegion(outName)
    ...
    c1.Print(outName+"_noStat.png")
    c1.Print(outName+"_noStat.pdf")
